...
async function getMatrixifySeatCovers(start: number, end: number) {
  const { data, error } = await supabaseCoverlandSizeChart.rpc("get_shopify_matrixify_seat_covers_draft").range(start, end);
...
async function updateSeatCoverMatrixifyChart(auth: any) {
  ...
  const SHEETS_ID = "1I7buhlkC16j16TKw59SwWve4dMHnEsfYq5es536lvKE";
  const sheetName = "matrixify";
