...
async function getMatrixifyFloorMatsFullSet(start: number, end: number) {
  const { data, error } = await supabaseCoverlandSizeChart.rpc("get_shopify_matrixify_floor_mats_full_set_only").range(start, end);
...
async function updateFloorMatsFullSetMatrixifyChart(auth: any) {
  ...
  const sheetName = "floor_mats_matrixify_full_set_only";
  ...
  while (true) {
    ...
    const { data, error } = await getMatrixifyFloorMatsFullSet(start, start + BATCH_SIZE - 1);
...
authorize().then(updateFloorMatsFullSetMatrixifyChart).catch(console.error);
